...
export function fnv1aStep(h: number, code: number): number {
  h ^= code;
  // h *= 16777619 (but with shifts to stay in 32-bit)
  return (h + ((h << 1) + (h << 4) + (h << 7) + (h << 8) + (h << 24))) >>> 0;
}

export function fnv1aSeed(): number {
...
function hashNumber(h: number, n: number): number {
  ...
  for (let i = 0; i < 8; i++) h = fnv1aStep(h, _numView.getUint8(i));
  return h;
}

export function hashValue(h: number, v: unknown): number {
  switch (typeof v) {
    case 'string': h = fnv1aStep(h, 0x01); return hashString(h, v);
    case 'number': h = fnv1aStep(h, 0x02); return hashNumber(h, v);
    case 'boolean': h = fnv1aStep(h, 0x03); return fnv1aStep(h, v ? 1 : 0);
    case 'undefined': h = fnv1aStep(h, 0x04); return h;
    case 'object':
      if (v === null) { h = fnv1aStep(h, 0x05); return h; }
      if (Array.isArray(v)) {
        h = fnv1aStep(h, 0x06);
      ...
      h = fnv1aStep(h, 0x07);
      const k = Object.keys(v as Record<string, unknown>).sort(); // fast; avoids localeCompare
      for (let i = 0; i < k.length; i++) {
        h = fnv1aStep(h, 0x3A); // ':'
        h = hashString(h, k[i]);
        h = fnv1aStep(h, 0x3D); // '='
        h = hashValue(h, (v as any)[k[i]]);
      }
      return h;
    case 'symbol':
      h = fnv1aStep(h, 0x08);
    ...
    case 'function':
      ...
      h = fnv1aStep(h, 0x09);
      return hashString(h, (v as Function).name || '');
    default:
      return h;
  }
}
